...
namespace Tours.Controllers
{
    ...
    public class Add_TourController : ControllerBase
    {
       ...
       // [Authorize(Roles = "Admin")]
        ...
        [HttpGet("tours_without_itineraries")]
        public async Task<ActionResult<IEnumerable<Add_Tour>>> GetToursWithoutItineraries()
        {
            var toursWithoutItineraries = await service.GetToursWithoutItineraries();
            return Ok(toursWithoutItineraries);
        }
        ...
        public async Task<ActionResult<Add_Tour>> Get_Tourby_id(int tour_id)
        {
            try
            {
                var tour = await service.Get_toursbyid(tour_id);
                if (tour == null)
                {
                    return NotFound();
                }
        ...
        [HttpGet("tour_owner_id/{tour_owner_id}")]
        public async Task<ActionResult<Add_Tour>> Get_Tours_ownerbyid(int tour_owner_id)
        {
            try
            {
                var tour = await service.Get_toursofagency(tour_owner_id);
                if (tour == null)
                {
                    return NotFound();
                }
