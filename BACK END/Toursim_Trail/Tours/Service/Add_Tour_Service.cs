...
namespace Tours.Service
{
    public class Add_Tour_Service:IAdd_Tours
    {
        ...
        public async Task<IEnumerable<Add_Tour>> GetAllTours()
        {
            try
            {
                var Tours = await context.Add_Tours
           .Include(x => x.feedbacks)
           .Include(x => x.tour_Agency) // Include the related Tour_Agency entities
           .ToListAsync();
                return Tours;
        ...
        public async Task<List<Add_Tour>> GetToursWithoutItineraries()
        {
            var toursWithoutItineraries = await context.Add_Tours
                .Where(tour => !context.Itineraries.Any(itinerary => itinerary.Add_Tour.tour_id == tour.tour_id))
                .ToListAsync();

            return toursWithoutItineraries;
        }
        ...
        public async Task<List<Add_Tour>> Get_toursofagency(int tour_owner_id)
        {
            try
            {
                var tours = await context.Add_Tours
                                   .Include(tour => tour.tour_Agency)
                                   .Where(tour => tour.tour_Agency.tour_owner_id == tour_owner_id)
                                   .ToListAsync();
                return (tours);
            }
            catch (Exception ex)
            {
                throw new Exception("Failed to retrieve the Tours by ID.", ex);
            }
        }
        //To get the tour by id 
        public async Task<List<Add_Tour>> Get_toursbyid(int tour_id)
        {
            try
            {
                var tours  =  await context.Add_Tours.Where(d => d.tour_id == tour_id).ToListAsync();
                return tours;
            }
            catch (Exception ex)
            {
